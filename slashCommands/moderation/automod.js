    ...
    async execute(interaction) {
        ...
        if (group === 'word') {
            ...
            if (sub === 'remove') {
                ...
                await updateGuildConfig(guild.id, { automodBannedWordList: wordList.filter(w => w !== word) });
            ...
            if (sub === 'list') {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(wordList.map(w => `\`${w}\``).join(', '));
        ...
        if (sub === 'action') {
            ...
            const update = { automodAction: type };
            ...
            const label = type === 'timeout'
                ? `Delete + Timeout (${formatDuration(update.automodTimeoutSeconds * 1000)})`
        ...
        if (sub === 'view') {
            ...
            const actionLabels = { delete: 'Delete only', warn: 'Delete + Warn', timeout: 'Delete + Timeout' };
