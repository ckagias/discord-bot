import { SlashCommandBuilder, EmbedBuilder, version, ChatInputCommandInteraction, Client } from 'discord.js';
const os = require('os');
...
function buildFeatureColumns(client: Client) {
...
function getLavalinkVersion(client: Client) {
    const node = (client.lavalink as any)?.nodeManager?.nodes?.values().next().value;
