    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (sub === 'global') {
            ...
            const embed = new EmbedBuilder()
                ...
                .setAuthor({ name: user.tag, iconURL: user.displayAvatarURL({ dynamic: true } as any) })
                ...
                .setImage(user.displayAvatarURL({ dynamic: true, size: 4096 } as any))
        ...
        if (sub === 'server') {
            ...
            const serverAvatar = member.avatarURL({ dynamic: true, size: 4096 } as any);
            if (!serverAvatar) {
