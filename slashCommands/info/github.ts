        ...
        try {
            ...
            try {
                ...
                const svg = svgRes.data
                    .replace(/fill:([^;]+)/g, (_: string, c: string) => `fill:${colorMap[c.trim()] ?? c}`)
                    .replace(/x="(\d+)"/g, (_: string, n: string) => `x="${parseInt(n) - 25}"`)
                ...
                files = [attachment];
            } catch (e) { logger.error('chart error:', (e as Error).message); }
