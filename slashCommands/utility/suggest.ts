import { SlashCommandBuilder, PermissionFlagsBits, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, MessageFlags, ChatInputCommandInteraction, Client } from 'discord.js';
...
function suggestionEmbed(suggestion: any) {
...
function voteRow(suggestionId: unknown, reviewable = true) {
...
function staffRow(suggestionId: unknown) {
...
async function applyStatus(client: Client, suggestion: any, status: string, staffId: string) {
    ...
    const message = await (channel as any).messages.fetch(suggestion.messageId).catch(() => null);
    if (!message) return;
...
module.exports = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (sub === 'submit') {
            ...
            const embed = suggestionEmbed(suggestion);
            const msg = await (channel as any).send({ embeds: [embed], components: [voteRow(suggestion._id)] });
