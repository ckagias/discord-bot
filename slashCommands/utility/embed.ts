import { SlashCommandBuilder, PermissionFlagsBits, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, EmbedBuilder, MessageFlags, ChatInputCommandInteraction } from 'discord.js';
...
module.exports = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (sub === 'create') {
            ...
            if (!(channel as any).permissionsFor(botMember).has(PermissionFlagsBits.SendMessages))
        ...
        if (sub === 'edit') {
            ...
            const message = await (interaction.channel as any).messages.fetch(messageId).catch(() => null);
...
function buildMainModal(customId: string, existing: any = null) {
...
function buildFieldsModal(customId: string, existingFields: any[] = []) {
    const modal = new ModalBuilder()
        .setCustomId(customId)
...
function parseMainFields(interaction: any) {
...
function parseFieldLines(interaction: any) {
    const fields = [];
    for (let n = 1; n <= 5; n++) {
...
function buildEmbed({ title, description, color, footerText, imageUrl }: { title?: string; description?: string; color: number; footerText?: string; imageUrl?: string }, fields: any[] = []) {
