...
module.exports = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (opponent) return handlePvpCoinflip(interaction, guess, bet, opponent);
        return handleVsBotCoinflip(interaction, guess, bet);
    },
};

async function handlePvpCoinflip(interaction: ChatInputCommandInteraction, guess: string | null, bet: number | null, opponent: any) {
...
async function handleVsBotCoinflip(interaction: ChatInputCommandInteraction, guess: string | null, bet: number | null) {
    const result = Math.random() < 0.5 ? 'heads' : 'tails';
    const emoji = result === 'heads' ? '🪙' : '🟤';
