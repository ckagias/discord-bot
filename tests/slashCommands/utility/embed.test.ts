...
function makeInteraction({ sub, channel = null, messageId = 'msg1' }: { sub: string; channel?: any; messageId?: string }) {
    return {
        options: {
...
describe('embed command', () => {
    ...
    describe('create', () => {
        ...
        test('defaults to the current channel when no channel option is given', async () => {
            const interaction: any = makeInteraction({ sub: 'create', channel: null });
    ...
    describe('edit', () => {
        test('rejects when the message cannot be found', async () => {
            ...
            (interaction.channel.messages.fetch as jest.Mock).mockResolvedValue(null);
        ...
        test('rejects a message not posted by the bot', async () => {
            const interaction = makeInteraction({ sub: 'edit' });
            (interaction.channel.messages.fetch as jest.Mock).mockResolvedValue({ author: { id: 'someoneElse' }, embeds: [] });
        ...
        test('rejects a bot message with no embed', async () => {
            const interaction = makeInteraction({ sub: 'edit' });
            (interaction.channel.messages.fetch as jest.Mock).mockResolvedValue({ author: { id: 'bot1' }, embeds: [] });
        ...
        test('shows the pre-filled edit modal for a valid target message', async () => {
            const interaction = makeInteraction({ sub: 'edit' });
            (interaction.channel.messages.fetch as jest.Mock).mockResolvedValue({
                author: { id: 'bot1' },
...
describe('embed helper exports', () => {
    ...
    describe('parseMainFields', () => {
        function makeModalInteraction(values: Record<string, string>) {
