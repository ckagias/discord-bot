...
function makeInteraction({ sub, month = null, day = null, year = null }: { sub: string; month?: number | null; day?: number | null; year?: number | null }) {
    return {
        options: {
            getSubcommand: jest.fn().mockReturnValue(sub),
            getInteger: jest.fn((name) => ({ month, day, year } as Record<string, number | null>)[name]),
        },
        guild: { id: 'g1' },
        user: { id: 'u1' },
