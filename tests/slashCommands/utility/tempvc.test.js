...
function makeMember({ id = 'user1', voiceChannel = null, hasPermission = true } = {}) {
    return {
        id,
...
function makeInteraction({ sub, member = makeMember(), category = null, name = 'My VC', locked = null, limit = null, targetUser = null } = {}) {
    return {
        options: {
...
describe('tempvc command', () => {
    ...
    describe('create', () => {
        ...
        test('reports failure when channel creation fails', async () => {
            ...
            getGuildConfig.mockResolvedValue({});
            interaction.guild.channels.create.mockRejectedValue(new Error('missing permission'));
    ...
    describe('invite', () => {
        ...
        test('rejects when the voice channel is not a registered temp VC', async () => {
            ...
            const interaction = makeInteraction({ sub: 'invite', member });
        ...
        test('rejects inviting yourself', async () => {
            const member = makeMember({ id: 'user1', voiceChannel: { id: 'vc1' } });
            const interaction = makeInteraction({ sub: 'invite', member, targetUser: { id: 'user1' } });
        ...
        test('grants connect permission and DMs the invited user', async () => {
            ...
            const interaction = makeInteraction({ sub: 'invite', member, targetUser: { id: 'target1', username: 'Target' } });
            interaction.client.tempVCs = new Map([['vc1', 'user1']]);
