...
function makeInteraction({ sub, hasPermission = true, text = 'add more music sources' } = {}) {
    return {
        options: {
...
describe('suggest command', () => {
    ...
    describe('submit', () => {
        ...
        test('posts the suggestion and persists it', async () => {
            ...
            interaction.guild.channels.fetch.mockResolvedValue(channel);
            ...
            const [{ components }] = channel.send.mock.calls[0];
            expect(components).toHaveLength(1);
            const buttonLabels = components[0].components.map(c => c.data.label);
