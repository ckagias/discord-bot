...
function makeCollection(items: any[]) {
    ...
    map.filter = (fn: any) => makeCollection([...map.values()].filter(fn));
    return map;
}
...
function makeInteraction({ amount = 10, user = null, bots = null, contains = null, attachments = null }: { amount?: number; user?: any; bots?: boolean | null; contains?: string | null; attachments?: boolean | null } = {}) {
    return {
        options: {
...
describe('purge command', () => {
    ...
    test('filters by user and reports when nothing matches', async () => {
        ...
        (interaction.channel.messages.fetch as jest.Mock).mockResolvedValue(makeCollection([makeMessage({ author: { id: 'user1', bot: false } })]));
    ...
    test('filters by user and deletes matches', async () => {
        ...
        (interaction.channel.messages.fetch as jest.Mock).mockResolvedValue(makeCollection([
    ...
    test('filters by content substring case-insensitively', async () => {
        ...
        (interaction.channel.messages.fetch as jest.Mock).mockResolvedValue(makeCollection([makeMessage({ content: 'hello world' })]));
    ...
    test('reports a permission/age error when bulkDelete throws', async () => {
        ...
        (interaction.channel.bulkDelete as jest.Mock).mockRejectedValue(new Error('too old'));
