...
function makeInteraction({ sub, user = null, levelValue = 5, xp = null, hasPermission = true } = {}) {
    return {
        options: {
...
describe('level command', () => {
    ...
    describe('set', () => {
        ...
        test('sets the level and xp, and grants backfilled level roles', async () => {
            ...
            interaction.guild.roles.cache.get.mockImplementation((id) => ({ id }));
        ...
        test('does not re-grant a level role the member already has', async () => {
            ...
            interaction.guild.members.fetch.mockResolvedValue(member);
            interaction.guild.roles.cache.get.mockReturnValue({ id: 'role3' });
