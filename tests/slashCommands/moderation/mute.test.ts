...
function makeCollection(items: any[] = []) {
    const map: any = new Map(items.map((item, i) => [item.id ?? i, item]));
...
function makeInteraction({ target, duration = null, reason = null, muteRole = { id: 'role1' } }: { target: any; duration?: string | null; reason?: string | null; muteRole?: any }) {
    return {
        options: {
